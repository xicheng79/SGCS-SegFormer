...
def set_random_seed(seed, deterministic=False):
    ...
    参数:
        ...
        deterministic (bool): 是否为 CUDNN 后端设置确定性选项，
            即将 `torch.backends.cudnn.deterministic` 设置为 True，
            并将 `torch.backends.cudnn.benchmark` 设置为 False。
            默认值: False。
    """
    random.seed(seed)
    np.random.seed(seed)
...
def train_segmentor(model,
    ...
    if distributed:
        # 当按轮次进行分布式训练时，使用 `DistSamplerSeedHook` 为每一轮的分布式采样器设置不同的种子，
        # 这样每一轮都会对数据集进行洗牌，避免过拟合。

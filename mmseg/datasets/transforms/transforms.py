...
class BioMedical3DRandomCrop(BaseTransform):
    ...
    - img (np.ndarray): 生物医学图像，形状为 (N, Z, Y, X)，
        N 是模态数量，数据类型为 float32。
    - gt_seg_map (np.ndarray, 可选): 生物医学语义分割掩码，
        形状为 (Z, Y, X)。
    ...
    参数:
        crop_shape (Union[int, Tuple[int, int, int]]): 裁剪后的预期大小，格式为 (z, y, x)。如果设置为整数，
            则裁剪的宽度和高度都等于该整数。
        keep_foreground (bool): 如果 keep_foreground 为 True，它将随机采样一个前景类别的体素，
            并将其作为裁剪边界框的中心。默认为 True。
    """
    ...
    def generate_margin(self, results: dict) -> tuple:
        ...
        如果 keep_foreground 为 True，它将随机采样一个前景类别的体素，并将其作为边界框的中心，
        然后返回边界框和图像之间的边距。
        如果 keep_foreground 为 False，它将返回裁剪形状和图像形状之间的差异。

        参数:
            results (dict): 来自加载流水线的结果字典。

        返回:
            tuple: 裁剪边界框和图像在 3 个维度上的边距。
        """
...
class BioMedicalGaussianBlur(BaseTransform):
    """为图像添加具有随机 sigma 值的高斯模糊。

    改编自 https://github.com/MIC-DKFZ/batchgenerators/blob/7651ece69faf55263dd582a9f5cbd149ed9c3ad0/batchgenerators/transforms/noise_transforms.py#L81 # noqa:E501

    版权归德国癌症研究中心 (DKFZ) 所有
    遵循 Apache 许可证，版本 2.0

    必需键:

    - img (np.ndarray): 生物医学图像，形状为 (N, Z, Y, X)，
            N 是模态数，数据类型为 float32。
    ...
    参数:
        sigma_range (Tuple[float, float]|float): 随机选择 sigma 值的范围。默认为 (0.5, 1.0)。
        prob (float): 对每个样本应用高斯模糊的概率。默认为 0.2。
        prob_per_channel  (float): 对图像的每个通道（轴 N）应用高斯模糊的概率。默认为 0.5。
        different_sigma_per_channel (bool): 是否为图像的每个通道（轴 N）使用不同的 sigma 值。默认为 True。
        different_sigma_per_axis (bool): 是否为图像的 Z、X 和 Y 轴使用不同的 sigma 值。默认为 True。
    """
...
class BioMedicalRandomGamma(BaseTransform):
    ...
    - img (np.ndarray)：形状为 (N, Z, Y, X) 的生物医学图像，
        N 是模态数，数据类型为 float32。
    ...
    参数：
        prob (float)：执行此变换的概率。默认值：0.5。
        gamma_range (Tuple[float])：伽马值的范围。默认值：(0.5, 2)。
        invert_image (bool)：在应用伽马增强之前是否反转图像。默认值：False。
        per_channel (bool)：是否对每个通道单独执行变换。默认值：False。
        retain_stats (bool)：伽马变换会改变图像块中数据的均值和标准差。如果 retain_stats=True，数据将被变换以匹配伽马增强之前的均值和标准差。默认值：False。
    """
...
class BioMedical3DPad(BaseTransform):
    """对生物医学 3D 图像和生物医学 3D 语义分割图进行填充。

    必需键:

    - img (np.ndarry): 生物医学图像，默认形状为 (N, Z, Y, X)，
        N 是模态数量。
    - gt_seg_map (np.ndarray, 可选): 生物医学分割图，默认形状为
        (Z, Y, X)。
    ...
    - pad_shape (Tuple[int, int, int]): 填充后的形状。

    参数:
        pad_shape (Tuple[int, int, int]): 固定的填充大小。
            期望的填充形状为 (Z, Y, X)。
        pad_val (float): 生物医学图像的填充值。
            填充模式设置为 "constant"。填充区域要填充的值。默认值: 0。
        seg_pad_val (int): 生物医学 3D 语义分割图的填充值。
            填充模式设置为 "constant"。填充区域要填充的值。默认值: 0。
    """
...
class BioMedical3DRandomFlip(BaseTransform):
    ...
    改编自 https://github.com/MIC-DKFZ/batchgenerators/blob/master/batchgenerators/transforms/spatial_transforms.py # noqa:E501

    版权所有 2021 医学图像计算部门、德国癌症研究中心 (DKFZ) 以及应用计算机视觉实验室、亥姆霍兹成像平台。
    遵循 Apache-2.0 许可证。

    必需键:

    - img (np.ndarry): 默认形状为 (N, Z, Y, X) 的生物医学图像，
        N 是模态的数量。
    - gt_seg_map (np.ndarray, 可选): 默认形状为 (Z, Y, X) 的生物医学分割图。
